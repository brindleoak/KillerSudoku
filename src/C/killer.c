...
//read the killer rules from a csv file. build "rules" array with the rule for each cell 
int readRules() {
	int i = 0;
	char line[128];
	char *token = NULL;
	...
	while (fgets(line, sizeof(line), fp) != NULL && i < 50)	{
		...
		int maxCell = 1;
		for (int j = 1; j < 5; j++)
			if (rule[j] > rule[maxCell])
				maxCell = j;

		for (int k = 0; k < 5; k++)
			rules[rule[maxCell]][k] = rule[k];
	}
	fclose(fp);
}

//for a given board, cell and value, check if this breaks the killer rules
...
// check the killer rules. then check the standard sudoku rules
