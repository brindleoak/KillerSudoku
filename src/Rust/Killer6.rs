...
fn main() {
    ...
    fn check_row(row_to_check: &[usize], vals: &HashMap<usize,&Vec<usize>>, newval: &usize) -> bool {
        let pos: usize;
        ...
        if vals.contains_key(&(pos)) {
            let selected_parm: Vec<usize> = vals.get(&(pos)).unwrap().to_vec();
            let sub_parm = &selected_parm[1..selected_parm.len()];
            let total = sub_parm
                .iter()
                .map(|&cell_position| row_to_check[cell_position])
                .fold(*newval, |total, next| total + next);
        ...
        if in_board.iter().position(|&x| x == 0).is_none() {
            ...
            while y < 10 {
                if !excluded_numbers.contains(&y) {
                    if check_row(in_board, vals, &y) {
						let new_board: Vec<usize> =
