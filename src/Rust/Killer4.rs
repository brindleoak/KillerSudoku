...
//for (&i, &_val ) in parms.iter() {
    ...
    fn recursive_check(in_board: &[usize], vals: &HashMap<usize,&Vec<usize>>, grid:&[[usize; 81]; 81]) -> bool {
            ...
            while y < 10 {
                if !excluded_numbers.contains(&y) {
                    ...
                    if check_row(&mut new_board, vals) {
                        if i < 80 {
                            recursive_check(&new_board, &vals, &grid);
                        } else {
    ...
    recursive_check(&initial_row, &parms, &grid_rules);
}
